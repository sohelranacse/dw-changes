...
class Linkedin
{
    ...
    private function getUrlAuth($type='')
    {
        /*
        ...
        */
        $and = ($type) ? "&type=$type" : '';
        return 'social_login.php?module='.self::$nameSocial.$and;
    }
    ...
    public function getUserEmail()
    {
        ...
        if(isset($userInfo['emailAddress'])){
    ...
    public function loginRedirectUrl($type='')
    {
        $url = '';
        $url = $this->getUrlAuth($type);
        return $url;
    }
    ...
    public function oAuthApi($type)
    {
        ...
        $currentUrl = Social::getCallbackUrl($nameSocial, $type);
            ...
            if(($success = $client->Process()))
            {
                ...
                if($accessTokenData && isset($accessTokenData['value'])) {
                    ...
                    if(is_array($userinfo)) {
                        ...
                        if(isset($userinfo['localizedFirstName']))
                            $user['firstName'] = $userinfo['localizedFirstName'];

                        if(isset($userinfo['localizedLastName']))
        ...
        if($success)
        {
            ...
            // NEW CODE START
            // CHECK FOR BOTH - LOGIN/REGISTER
            if(checkBanUserByEmail($userInfo['emailAddress'])) // User Account is Banned
                redirect("index?message=banned_account");

            // CHECK FOR REGISTER - User Account Exist
            // if($type !== "social_login" && $type !== "link_account" && checkActiveUserByEmail($userInfo['emailAddress']))
                // redirect("login?message=exist_account");
            // NEW CODE START END

            set_session($nameSocial.'_user_info', $userInfo);
            redirect("join_facebook.php?cmd=ln_login&type=$type");
        } else {
