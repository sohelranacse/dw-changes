...
class CJoinPage extends CHtmlBlock {
    ...
    function parseBlock(&$html)
    {
        ...
        if($by && $by === 'phone') { // using in main version
            $html->setvar('new_password', generateSecurePassword());
        }
        ...
        $html->setvar("signup_as", $signup_as);

        if(isset($_GET['type']))
            $html->setvar("signup_as", get_param('type'));
        ...
        // USER RESTRICT MESSAGE
        $message = get_param('message');
        if($message) {
            $html->setvar('messagetitle', l($message));
            $html->parse('user_error_message', false);
        }
