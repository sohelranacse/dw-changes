...
class CHeader extends CHtmlBlock {
    ...
    function parseBlockImpact(&$html)
    {
        ...
        if ($html->varExists('user_photo_default_url') && $guid) {
            $html->setvar('user_photo_default_url', User::getPhotoDefault($guid, 'b').$g['site_cache']["cache_version_param"]);
        }
        ...
        if ($guid) {
            ...
            $blSocialCandidate = 'candidate_register';
            if ($html->blockExists($blSocialCandidate)) {
                Social::parse($html, $blSocialCandidate);
            }

            $blSocialMatchmaker = 'matchmaker_register';
            if ($html->blockExists($blSocialMatchmaker)) {
                Social::parse($html, $blSocialMatchmaker);
            }
        ...
        if($html->varExists('user_allowed_feature')) {
            ...
            // CUSTOMIZATION
            if(guid() && email_verfication_required()) {
                $user_allowed_feature = json_decode(User::accessСheckFeatureSuperPowersGetList());
                $min_number_upload_photos = $user_allowed_feature->min_number_upload_photos;
                $email_verification = $user_allowed_feature->email_verification;
            
                if($email_verification && !$min_number_upload_photos && $p !== 'restricted.php')
                    redirect('./restricted?cmd=email_verification');
    ...
    function parseBlock(&$html)
    {
        ...
        if(guid()) {
            $html->setvar('check_uploaded_photo', check_uploaded_photo());
            $html->parse('check_uploaded_photo', false);

            if(!check_uploaded_photo()) {

                if (isset($g['mobile']) && $p == "profile_view.php" && !get_param('user_id')) {
                    // nothing
                } else if (
                    !isset($g['mobile']) && 
                    ($p == "profile_view.php" && get_param('name_seo') == '') || $p == "group_users_edit.php") 
                {
                    // nothing
                } else
                    $html->parse('view_upload_photo_alert', false);
            }
        }
...
*/
        ...
        if (isset($g_user['role']) && $g_user['role'] == "group_admin") {
            $html->parse('matchmakersLabel', false);
        }

        // DISABLE NOW
        /*if (isset($g_user['user_id']) && $g_user['user_id']) {
            $html->parse('service_worker_enable', false);
        }*/
        ...
        */
        ...
        if($html->blockExists('facebook_invite')) {
            $sql = 'SELECT `status`
                FROM `pages`
                WHERE `menu_title` = "column_narrow_invite" AND `set` = "impact"';
            if(DB::result($sql)) {
