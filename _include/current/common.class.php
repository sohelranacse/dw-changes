...
Class Common {
    ...
    static function sendAutomail($lang, $email, $type, $vars = array(), $emailAuto = false, $dbIndex = DB_MAX_INDEX, $isGetData = false)
    {
        ...
        if ($emailAuto) {
            ...
            if(Common::isValidArray($vars)) {
                ...
                if($type == 'contact')
                    $data['url'] = 'https://match.deshiwedding.com';
    ...
    static function toHomePage($role='')
    {
        ...
        // NEW CODE BY SOHEL
        if(
            get_param("type") && get_param("type") == "social_login" && 
            $role && $role == "group_admin"
        ) {
            $ref_link = "group_users";
        }
   ...
   static function getLoginPage()
    ...
    static function listCountries($selected, $firstDefault = false, $list = false, $isFirstValueEmpty = true, $setCountryDefault = true)
    {
        $countries = '';
...
JS;
    ...
    static function getSeoSite($urls, $uid, $userInfo = null, $noPlaceCity = false, $groupId = false)
    {
        ...
        if ($uid) {
            ...
            if($userInfo['city'])
                $vars['location'] = $userInfo['city'] ? l($userInfo['city']) : l($userInfo['country']);
        }
